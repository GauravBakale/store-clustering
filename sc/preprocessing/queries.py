...
def store_cluster_query(store_clustering_features):
    ...
    query = f"select store_code,{inner_query} from {models.Variable.get('gcp_project', default_var='')}.{models.Variable.get('dataset')}.store_master"
    return query


def lost_sales_imputation_query():
    impute_features = eval(models.Variable.get("impute_features"))
    lsi_channels = eval(models.Variable.get("lsi_channel"))
    active_stores_cutoff=eval(models.Variable.get('active_stores_cutoff'))
    sell_through_cutoff=eval(models.Variable.get('sell_through_cutoff'))
    ...
    on_condition_sc= [f" a.{i} = b.{i}" for i in eval(models.Variable.get("level_of_store_clustering"))]
    ...
    where_backfill= '' if models.Variable.get("ingestion_type") != 'periodic' or eval(models.Variable.get("main_master_schema_change")) else models.Variable.get("main_master_backfill")
    final_query = f"""
                        create or replace table {models.Variable.get('gcp_project', default_var='')}.{models.Variable.get('dataset')}.master_table_imputed_interim as
                        ...
                        from  {models.Variable.get('gcp_project', default_var='')}.{models.Variable.get('dataset')}.master_table  a
                        ...
                        from {models.Variable.get('gcp_project', default_var='')}.{models.Variable.get('dataset')}.store_clustering)  b 
...
def price_imputation_query():
    hierarchy_list = eval(models.Variable.get("all_hierarchies_reverse"))
    ...
    if models.Variable.get("ingestion_type") != 'periodic' or eval(models.Variable.get("main_master_schema_change")):
        create_or_insert_query_start= f"""
            create or replace table {models.Variable.get('gcp_project', default_var='')}.{models.Variable.get('dataset')}.master_input_imputed 
            partition by {master_table_imputed_partition_column} as"""
        create_or_insert_query_end=''
    else: 
        master_cols_query = f"""SELECT column_name
                FROM {models.Variable.get('gcp_project', default_var='')}.{models.Variable.get('dataset')}.INFORMATION_SCHEMA.COLUMNS
                WHERE table_name = 'master_input_imputed' """
        master_cols_df = pd.read_gbq(master_cols_query, project_id=models.Variable.get('gcp_project', default_var=''), dialect='standard')
        master_table_column_names = list(master_cols_df["column_name"])
        create_or_insert_query_start=f"""
                    BEGIN
                    BEGIN TRANSACTION;
                    delete from {models.Variable.get('gcp_project', default_var='')}.{models.Variable.get('dataset')}.master_input_imputed where date > current_date()-{models.Variable.get("main_master_backfill")};
                    insert into {models.Variable.get('gcp_project', default_var='')}.{models.Variable.get('dataset')}.master_input_imputed ({','.join(master_table_column_names)})
    ...
    ( with base_table as (
        select *
        from {models.Variable.get('gcp_project', default_var='')}.{models.Variable.get('dataset')}.master_table_imputed_interim
    ),
     {temp_table_const},
    ...
    price_imputation as (
        select a.*except({price_imputed_list_const}),{models.Variable.get("price_impute_string")}
        from coalesce_discount_table a
        left join {models.Variable.get('gcp_project', default_var='')}.{models.Variable.get('dataset')}.product_master b
