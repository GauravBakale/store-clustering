...
def get_all_grids(
    n_clusters=None, selected_algorithms=None, algorithms_df=get_all_algorithms()
):
    ...
    if n_clusters:
        algorithms_df = algorithms_df.loc[algorithms_df["k_as_input"] == True]
    ...
    for index in default_algorithms.index.to_list():
        ...
        grid = GridSearchCV(
            ...
            param_grid=eval(default_algorithms.loc[index, "parametres"]),
    ...
    for idx, k_input, param in zip(
        ...
        custom_algorithms.parametres.to_list(),
    ):
        if k_input:
            if n_clusters:
...
def select_best_algo_clustering(input_df: pd.DataFrame, labels_df: pd.DataFrame):
    # TODO : accept the error as a metric one or more.
    ...
    error_metrics_df["rank_sum"] = (
        error_metrics_df["sil_rank"]
        + error_metrics_df["db_rank"]
        + error_metrics_df["ch_rank"]
    )
